...
class YoloService {
  ...
  // ── ВЫСОКОУРОВНЕВЫЙ МЕТОД ДЛЯ DefectScannerScreen ─────────
  // Раньше здесь была пустая заглушка (`async {}`), которая ничего не
  // возвращала и не делала сетевой запрос — сканер дефектов фактически
  // никогда не отправлял фото на бэкенд. Теперь метод реально вызывает
  // YOLO API через analyzeImage() и собирает результат в YoloResult,
  // который ожидает defect_scanner_screen.dart.
  static Future<YoloResult> analyzeDefect(File image) async {
    final detections = await analyzeImage(image);
    final category = recommendSpecialty(detections);
    final recommendation =
        detections.isNotEmpty ? detections.first.suggestion : null;
...
/// Обёртка над результатом анализа для экрана сканера дефектов:
/// список найденных дефектов + рекомендуемая специализация мастера +
/// текстовая рекомендация по первому (наиболее уверенному) дефекту.
class YoloResult {
