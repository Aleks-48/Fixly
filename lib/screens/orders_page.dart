...
class _OrdersPageState extends State<OrdersPage>
  ...
  Future<void> _loadOrders() async {
    ...
    try {
      ...
      final role = context?.roleKey ?? userRole.value;
      final buildingId = context?.buildingId;

      dynamic query = _supabase
          .from('tasks')
          .select(
              '*, profiles!tasks_user_id_fkey(full_name, apartment_number, avatar_url)')
      ...
      } else if (role == 'chairman' || role == 'manager' || role == 'admin') {
              ...
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText:
                        lang == 'ru' ? 'Поиск заявки...' : 'Өтінімді іздеу...',
  ...
  Widget _buildOrderCard(
    ...
    final clientName = profile['full_name']?.toString() ??
        order['customer_name']?.toString() ??
        '';
    final apt = profile['apartment_number']?.toString() ??
        order['apartment']?.toString() ??
        '';
            ...
            if (address.isNotEmpty || apt.isNotEmpty)
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      apt.isNotEmpty ? '$address, кв. $apt' : address,
  ...
  String _appBarTitle(String role, String lang) {
    if (role == 'chairman' || role == 'manager' || role == 'admin')
      return lang == 'ru' ? 'Заявки дома' : 'Үй өтінімдері';
