...
// Раньше тут были пути screens/profile/... — несуществующая подпапка,
// из-за чего файл не компилировался и тянул за собой все chairman-экраны.
...
class _MainWrapperState extends State<MainWrapper> {
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              if (titleController.text.isNotEmpty) {
                try {
                  // ВАЖНО: раньше здесь был прямой insert без поля
                  // 'status'. VotingListScreen и chairman_home_screen
                  // фильтруют голосования строго через
                  // .eq('status', 'active') — без этого поля созданное
                  // голосование навсегда пропадало из всех списков
                  // ("призрачное" голосование, видимое только в БД).
                  // Теперь используем тот же VotingService.createProposal,
                  // что и остальные экраны создания голосований.
  ...
  Widget build(BuildContext context) {
    ...
    // ВАЖНО: раньше для роли 'chairman' на вкладках 0 и 1 показывались
    // OrdersPage и MastersListScreen — те же экраны, что и для остальных
    // ролей. Готовые ChairmanHomeScreen (дашборд с KPI, голосованиями,
    // объявлениями) и ChairmanMoreScreen (меню: аналитика, объявления,
    // голосования, мастера, база знаний, профиль) существовали в проекте,
    // но никогда не рендерились — председатель физически не мог до них
    // добраться. Это и есть основная причина "сломанных chairman-экранов"
    // после апдейта: экраны были рабочими, но не подключены к навигации.
    final List<Widget> pages = _userRole == 'chairman'
        ? [
            const ChairmanHomeScreen(),
            const ChairmanMoreScreen(),
